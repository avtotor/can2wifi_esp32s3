...
static void accept_new_clients(void) {
    ...
    for (int i = 0; i < MAX_CLIENTS; i++) {
        if (SysSettings.clientNodes[i].socket_fd < 0 || !SysSettings.clientNodes[i].connected) {
            if (SysSettings.clientNodes[i].socket_fd >= 0)
...
static void poll_clients(void) {
    ...
    for (int i = 0; i < MAX_CLIENTS; i++) {
        ...
        if (n > 0) {
            ...
            for (int j = 0; j < n; j++)
                gvret_process_byte(&wifiGVRET, buf[j]);
...
void wifi_manager_send_buffered_data(void) {
    size_t len = gvret_num_available_bytes(&wifiGVRET);
    if (len == 0) return;
    uint8_t *buf = gvret_get_buffered_bytes(&wifiGVRET);
    for (int i = 0; i < MAX_CLIENTS; i++) {
        if (SysSettings.clientNodes[i].socket_fd >= 0 && SysSettings.clientNodes[i].connected) {
